...
import { CONTEXT_REACT_CONTEXTS_REGISTRY, EMPTY_STRING, MANAGER_REGEX } from "../internals";
...
export function getReactContext<T extends TAnyContextManagerConstructor>(
  ...
  if (CONTEXT_REACT_CONTEXTS_REGISTRY.has(managerConstructor)) {
    ...
    reactContext.displayName = "DS." + managerConstructor.name.replace(MANAGER_REGEX, EMPTY_STRING);
